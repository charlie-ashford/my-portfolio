...
projects.forEach(project => {
  ...
  card.innerHTML = `
    <div class="project-meta">
      ...
      <span>${project.link ? 'open' : 'private'}</span>
    </div>
    <h3>${project.title}</h3>
    <p>${project.description}</p>
    ${
      project.link
        ? `<a class="project-link" href="${project.link}" target="_blank" rel="noopener">Visit</a>`
        : `<span class="project-link">Internal</span>`
    }
  `;
...
workItems.forEach(item => {
  ...
  card.innerHTML = `
    ...
    <p class="project-meta">${item.date}</p>
  `;
