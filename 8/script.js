  ...
  stop.innerHTML = `
    ...
    <article class="route-card">
      <div class="route-meta">
        ...
        <span>${project.link ? 'open route' : 'private build'}</span>
      </div>
      <h3>${project.title}</h3>
      <p>${project.description}</p>
      ${
        project.link
          ? `<a class="route-link" href="${project.link}" target="_blank" rel="noopener">Visit checkpoint</a>`
          : ''
      }
    </article>
  `;
...
workItems.forEach(item => {
  ...
  card.innerHTML = `
    ...
    <p class="route-meta">${item.date}</p>
  `;
